import "server-only";
/**
 * Budget read models: what each envelope has taken so far this period.
 *
 * Queries, not use cases. They join budgets to the spending that landed
 * against them and hand back a row per screen; the writes that create and
 * change a budget are `@application/budgets`.
 */
import {
  startOfWeek,
  endOfWeek,
